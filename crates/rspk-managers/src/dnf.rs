//! DNF (Dandified YUM) package manager implementation.
//!
//! DNF is the successor to YUM and the default package manager for
//! Fedora, RHEL 8+, and Cent OS Stream. It provides improved dependency
//! resolution, better performance, and a Python-based plugin system.
...
/// The DNF package manager for Fedora, RHEL, and derivatives.
#[derive(Debug)]
pub struct Dnf
{
    cli_path: String,
...
impl Dnf
{
    /// Creates a new DNF manager instance.
    pub async fn new() -> Result<Self, Error>
    {
        let cli_path = which::which("dnf")
            .map_err(|_| Error::BinaryNotFound("dnf".to_string()))?
            .to_string_lossy()
            .to_string();
    ...
    /// Parses the output of `dnf list installed --quiet`.
    ///
    ...
    /// Parses the output of `dnf check-update --quiet`.
    ///
    /// Format: `package.arch    version    repo`
    ...
    ) -> Result<Option<String>, Error>
    {
        if let Ok(Some(name)) = self
            .repology
            .resolve_for_repo(abstract_name, "fedora")
            .await
        {
            return Ok(Some(name));
        }
...
impl PackageManager for Dnf
{
    ...
    async fn is_available(&self) -> Result<bool, Error>
    {
        CommandBuilder::new(&self.cli_path)
            ...
            .await
            .map(|_| true)
            .map_err(|_| Error::ManagerUnavailable("dnf".to_string()))
    }
    ...
    {
        // dnf check-update returns 100 when updates available, 0 otherwise
        let result = CommandBuilder::new(&self.cli_path)
            .arg("check-update")
            .arg("--quiet")
            .dry_run(ctx.dry_run)
    ...
    ) -> Result<Option<String>, Error>
    {
        ...
        {
            eprintln!(
                "[dry-run] resolving '{abstract_name}' via Repology and dnf \
                 info"
            );
            return Ok(Some(abstract_name.to_string()));
        }
...
mod tests
{
    ...
    {
        let output = "curl.x86_64                      7.81.0-1.fc38                      @updates\n";
        let packages = Dnf::parse_installed_output(output);
    ...
    fn test_parse_search_output()
    {
        ...
        let packages = Dnf::parse_search_output(output);
        assert_eq!(packages.len(), 1);
        assert_eq!(packages[0].id, "curl");
    }
}
