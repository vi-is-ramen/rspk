//! paru package manager implementation for Arch Linux with AUR support.
//!
//! This module provides support for managing packages via paru, an
//! AUR helper written in Rust that extends pacman with access to the
//! Arch User Repository (AUR). Paru provides a similar interface to
//! pacman while seamlessly handling both official repository packages
//! and AUR packages.
//!
//! Note: Paru manages privilege elevation internally, using sudo for
//! official repository operations while building AUR packages as the
//! current user.
...
/// The paru package manager.
///
/// An AUR helper written in Rust that extends pacman with access to
/// the Arch User Repository. Provides a unified interface for managing
/// both official repository packages and community-contributed AUR packages.
#[derive(Debug)]
pub struct Paru
{
    cli_path: String,
...
impl Paru
{
    /// Creates a new paru manager instance.
    ///
    /// # Errors
    ///
    /// Returns an error if the `paru` binary cannot be found on the system.
    pub async fn new() -> Result<Self, Error>
    {
        let cli_path = which::which("paru")
            .map_err(|_| Error::BinaryNotFound("paru".to_string()))?
            .to_string_lossy()
            .to_string();
    ...
    ///
    /// Paru produces colored output by default, which must be stripped
    ...
    /// Parses the output of `paru -Q` for installed packages.
    ///
    ...
    /// Parses the output of `paru -Qu` for outdated packages.
    ///
    ...
    /// Parses the output of `paru -Ss` for search results.
    ///
    /// Format:
    /// ```text
...
impl PackageManager for Paru
{
    ...
    async fn is_available(&self) -> Result<bool, Error>
    {
        CommandBuilder::new(&self.cli_path)
            ...
            .await
            .map(|_| true)
            .map_err(|_| Error::ManagerUnavailable("paru".to_string()))
    }
    ...
    ) -> Result<Option<String>, Error>
    {
        ...
        {
            eprintln!(
                "[dry-run] resolving '{abstract_name}' via AUR and paru -Ss"
            );
            return Ok(Some(abstract_name.to_string()));
        }
    ...
    {
        // Paru searches both official repos and AUR by default
        let output = CommandBuilder::new(&self.cli_path)
            .arg("-Ss")
            .arg("--color=never")
            .arg(query)
    ...
    {
        // Paru does not support installing specific versions via CLI.
        // It handles both repository and AUR packages transparently.
        //
        // Note: paru internally uses sudo for repo packages but builds
        // AUR packages as the current user. We don't pass with_sudo()
        // here because paru manages privilege elevation itself.
        let output = CommandBuilder::new(&self.cli_path)
            .arg("-S")
            .arg("--noconfirm")
            .arg("--color=never")
...
mod tests
{
    ...
    fn test_strip_ansi()
    {
        ...
        let stripped = Paru::strip_ansi(colored);
    ...
    fn test_parse_installed_output()
    {
        ...
        let packages = Paru::parse_installed_output(output);
...
"#;
        ...
        let packages = Paru::parse_outdated_output(output);
        assert_eq!(packages.len(), 2);
...
"#;
        ...
        let packages = Paru::parse_search_output(output);
        assert_eq!(packages.len(), 3);
    ...
    {
        let packages = Paru::parse_installed_output("");
        assert_eq!(packages.len(), 0);

        let packages = Paru::parse_outdated_output("");
        assert_eq!(packages.len(), 0);

        let packages = Paru::parse_search_output("");
        assert_eq!(packages.len(), 0);
    }
}
